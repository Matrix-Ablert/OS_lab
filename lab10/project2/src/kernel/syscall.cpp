...
void *realloc(void *address, int newSize) {
    return (void *)asm_system_call(8, (int)address, newSize);
}

void *syscall_realloc(void *address, int newSize) {
    PCB *pcb = programManager.running;
    if (pcb && pcb->pageDirectoryAddress)
    {
        return pcb->byteMemoryManager.reallocate(address, newSize);
    }
    return kernelByteMemoryManager.reallocate(address, newSize);
}
