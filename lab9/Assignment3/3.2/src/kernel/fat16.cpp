...
/**
 * getFileClusterCount - 返回指定文件占用的簇数
 *
 * 遍历方法：从文件的 firstCluster 出发，沿 FAT 链逐跳计数，
 * 直到遇到 EOF(0xFFF8) 或非法值为止。
 *
 * 例：文件 FAT 链为 2 → 5 → 8 → EOF，则返回 3。
 *     空文件 firstCluster==0，while 条件不满足直接返回 0。
 */
int FAT16::getFileClusterCount(const char *name)
{
    ...
    // 从起始簇沿 FAT 链逐跳计数；
    // 空文件 firstCluster 为 0（或 1，均为保留值），while 条件直接跳过，返回 0。
    int count = 0;
    uint16 cluster = entry.firstCluster;
    while (cluster >= 2 && cluster < FAT16_BAD)
    {
        count++;
        cluster = fatTable[cluster];  // 沿 FAT 链跳到下一簇
    }
...
/**
 * getFragmentation - 计算文件系统的碎片率
 *
 * 碎片率 = 不连续的 FAT 链接数 / 总 FAT 链接数
 *
 * 什么是"不连续的链接"？
 *   对于链接 current → next，如果 next != current + 1，
 *   说明这两个簇在物理上不是相邻的 → 记为一次碎片。
 *
 * 例：FAT 链为 5 → 6 → 9 → 10 → EOF
 *   5→6: 6 == 5+1, 连续 ✓
 *   6→9: 9 != 6+1, 碎片 ✗   (因为簇7,8被其他文件占用)
 *   9→10: 10 == 9+1, 连续 ✓
 *   结果：3 个链接中 1 个不连续，碎片率 = 1/3 = 33%
 *
 * 注意事项：
 *   - 只统计同一文件内部的簇间链接，不同文件间的间隔不算碎片
 *   - 单簇文件（没有链接）不计入，totalLinks==0 时直接返回 0.0
 *   - 跳过已删除条目（首字节 0xE5）
 */
float FAT16::getFragmentation()
{
    ...
    int fragmentedLinks = 0;  // 分子：不连续的链接数
    int totalLinks     = 0;   // 分母：所有文件内部的链接总数
    DirEntry entry;
    ...
    for (int i = 0; i < MAX_ROOT_ENTRIES; ++i)
    {
        ...
        // 对每个文件，沿 FAT 链检查相邻簇的连续性
        uint16 current = entry.firstCluster;
        while (current >= 2 && current < FAT16_BAD)
        {
            uint16 next = fatTable[current];
            // next 有效（不是 EOF 或 保留项）→ 存在一个链接
            if (next >= 2 && next < FAT16_BAD)
            {
                totalLinks++;
                // 关键判断：后一簇是否紧跟着前一簇
                // 例：current=6, next=9 → 9!=6+1 → 标记为碎片
                if (next != current + 1)
                    fragmentedLinks++;
            }
            current = next;  // 沿 FAT 链后移
        }
    }

    // 没有链接（如所有文件都是单簇或无文件），碎片率按 0 处理
    if (totalLinks == 0)
        return 0.0f;
