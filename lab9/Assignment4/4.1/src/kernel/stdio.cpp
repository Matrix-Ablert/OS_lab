...
static const uint16 SERIAL_COM1 = 0x3F8;

static void serial_initialize()
{
    asm_out_port(SERIAL_COM1 + 1, 0x00);
    asm_out_port(SERIAL_COM1 + 3, 0x80);
    asm_out_port(SERIAL_COM1 + 0, 0x03);
    asm_out_port(SERIAL_COM1 + 1, 0x00);
    asm_out_port(SERIAL_COM1 + 3, 0x03);
    asm_out_port(SERIAL_COM1 + 2, 0xC7);
    asm_out_port(SERIAL_COM1 + 4, 0x0B);
}
...
static void serial_write_char(char c)
{
    ...
    do
    {
        asm_in_port(SERIAL_COM1 + 5, &status);
    } while ((status & 0x20) == 0);

    asm_out_port(SERIAL_COM1, (uint8)c);
}
