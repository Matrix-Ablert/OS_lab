...
void STDIO::initialize()
{
    ...
    asm_out_port(0x3F8 + 3, 0x80); // 启用 DLAB
    asm_out_port(0x3F8 + 0, 0x01); // 波特率除数低字节 (115200)
    asm_out_port(0x3F8 + 1, 0x00); // 波特率除数高字节
    asm_out_port(0x3F8 + 3, 0x03); // 8N1
    asm_out_port(0x3F8 + 2, 0xC7); // 启用 FIFO
}

void STDIO::sendToSerial(uint8 c)
{
    // 等待发送缓冲区为空
    uint8 status;
    // 换行符先发送回车
    ...
    {
        do {
            asm_in_port(0x3F8 + 5, &status);
        } while ((status & 0x20) == 0);
        asm_out_port(0x3F8, '\r');
    }
    do {
        asm_in_port(0x3F8 + 5, &status);
    } while ((status & 0x20) == 0);
