...
void MemoryManager::initialize()
{
    ...
    // 初始化 Buddy System：将前 1024 个内核物理页交给 Buddy 管理（4MB）
    int buddyPages = 1024;
    if (buddyPages > kernelPages)
    {
        buddyPages = kernelPages;
    }
    // 从 bitmap 分配器中预占这些页，防止 bitmap 和 buddy 双重分配
    kernelPhysical.allocate(buddyPages);
    buddyAllocator.initialize(kernelPhysicalStartAddress, buddyPages);
}
...
int MemoryManager::buddyAllocatePages(int count)
{
    int addr = buddyAllocator.allocate(count);
    return addr;
}

void MemoryManager::buddyReleasePages(int addr, int count)
{
    buddyAllocator.release(addr);
}

void MemoryManager::printBuddyStatus()
{
    buddyAllocator.printInfo();
}