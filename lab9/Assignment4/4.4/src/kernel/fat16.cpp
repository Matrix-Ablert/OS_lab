...
void FAT16::fsck()
{
    ...
    static uint8 referenced[FAT_ENTRIES];
    ...
    for (int i = 0; i < MAX_ROOT_ENTRIES; ++i)
    {
        ...
        uint32 expectedClusters = (entry.fileSize + SECTOR_SIZE - 1) / SECTOR_SIZE;
        uint32 seenClusters = 0;
        uint16 cluster = entry.firstCluster;

        while (cluster >= 2 && cluster < FAT16_BAD && seenClusters < expectedClusters)
        {
            referenced[cluster]++;
            seenClusters++;

            if (seenClusters < expectedClusters && fatTable[cluster] == FAT16_FREE)
            {
                printf("  Chain error: %s stops early at cluster %d\n", name, cluster);
                chainErrors++;
                break;
            }
        ...
        if (seenClusters < expectedClusters)
        {
            printf("  Chain error: %s needs %d clusters, found %d\n", name, expectedClusters, seenClusters);
            chainErrors++;
        }
    }
    ...
    for (int i = 2; i < FAT_ENTRIES; ++i)
    {
        if (referenced[i] > 1)
        {
            printf("  Cross link: cluster %d referenced %d times\n", i, referenced[i]);
            crossLinks++;
        }
        if (fatTable[i] != FAT16_FREE && referenced[i] == 0)
        {
            printf("  Lost cluster: %d marked used but unreferenced\n", i);
            lostClusters++;
        }
    }

    printf("  Chain errors : %d\n", chainErrors);
    printf("  Lost clusters: %d\n", lostClusters);
    printf("  Cross links  : %d\n", crossLinks);
    printf("=== FSCK End ===\n");
}

void FAT16::injectFsckTestInconsistency()
{
    writeFile("broken.bin", "broken-data-needs-two-clusters", 700);
    writeFile("clean.bin", "clean", 5);
    writeFile("cross.bin", "cross", 5);

    int brokenIdx = findEntry("broken.bin");
    int cleanIdx = findEntry("clean.bin");
    int crossIdx = findEntry("cross.bin");
    DirEntry broken, clean, cross;
    readDirEntry(brokenIdx, &broken);
    readDirEntry(cleanIdx, &clean);
    readDirEntry(crossIdx, &cross);

    cross.firstCluster = clean.firstCluster;
    cross.fileSize = clean.fileSize;
    writeDirEntry(crossIdx, &cross);

    uint16 lost = allocateCluster();
    fatTable[broken.firstCluster] = FAT16_FREE;
    printf("Injected lost cluster: %d\n", lost);
    syncFAT();
}
