...
static char keyboardBuffer[128];
static int keyboardLength = 0;
static bool keyboardLineReady = false;

static char scanCodeToAscii(uint8 code)
{
...
extern "C" void c_keyboard_interrupt_handler()
{
    ...
    if (c == '\n')
    {
        keyboardBuffer[keyboardLength] = '\0';
        keyboardLineReady = true;
        printf("\n");
        return;
    }
    ...
    if (keyboardLength < 127)
    {
        keyboardBuffer[keyboardLength++] = c;
        printf("%c", c);
    }
}

bool keyboard_read_line(char *buffer)
{
    if (!keyboardLineReady)
        return false;

    strcpy(keyboardBuffer, buffer);
    keyboardLength = 0;
    keyboardLineReady = false;
    return true;
}
