...
private:
    ...
    bool splitPath(const char *path, char *dirName, char *baseName);
    int findEntryInDir(uint16 dirCluster, const char *name);
    int findFreeEntryInDir(uint16 dirCluster);
    void readDirEntryInDir(uint16 dirCluster, int index, DirEntry *entry);
    void writeDirEntryInDir(uint16 dirCluster, int index, const DirEntry *entry);
    bool isDirEmpty(uint16 dirCluster);
};
