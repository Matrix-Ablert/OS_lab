...
void ProgramManager::sleep(int ticks)
{
    if (ticks <= 0)
    {
        return;
    ...
    // 当前程序正在CPU上运行，不在ready队列中，可复用tagInGeneralList挂入sleep队列。
...
void ProgramManager::wakeupSleepingPrograms()
{
    ListItem *item = sleepingPrograms.front();

    while (item)
    {
        // 唤醒时会从sleep队列删除当前结点，必须先保存next避免遍历失效。
        ListItem *next = item->next;
        PCB *program = ListItem2PCB(item, tagInGeneralList);

        if (program->status == ProgramStatus::BLOCKED && program->sleepTicks > 0)
        {
            --program->sleepTicks;
        }
