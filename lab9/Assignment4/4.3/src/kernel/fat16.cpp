...
bool FAT16::writeFileOptimized(const char *name, const char *data, uint32 size)
{
    if (!mounted)
    {
        printf("Error: filesystem not mounted\n");
    ...
    static uint8 sectorBuf[SECTOR_SIZE * 8];

    while (cluster >= 2 && cluster < FAT16_BAD && bytesWritten < size)
    {
        uint16 startCluster = cluster;
        uint32 runCount = 1;
        while (fatTable[cluster] == cluster + 1 && runCount < 8)
        {
            cluster = fatTable[cluster];
            runCount++;
        }

        uint32 runBytes = runCount * SECTOR_SIZE;
        if (runBytes > size - bytesWritten)
            runBytes = size - bytesWritten;
        memset(sectorBuf, 0, sizeof(sectorBuf));
        memcpy((void *)(data + bytesWritten), sectorBuf, runBytes);
        disk->writeSectorsOptimized(clusterToSector(startCluster), runCount, sectorBuf);
...
int FAT16::readFileOptimized(const char *name, char *buffer, uint32 maxSize)
{
    int idx = findEntry(name);
    if (idx < 0)
    {
    ...
    while (cluster >= 2 && cluster < FAT16_BAD && bytesRead < readSize)
    {
        ...
        disk->readSectorsOptimized(clusterToSector(startCluster), runCount, sectorBuf);
        uint32 copySize = runCount * SECTOR_SIZE;
        if (copySize > readSize - bytesRead)
            copySize = readSize - bytesRead;
        memcpy(sectorBuf, buffer + bytesRead, copySize);
        bytesRead += copySize;
        cluster = fatTable[cluster];
    }
